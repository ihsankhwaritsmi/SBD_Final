...
-- Last modification date: 2023-12-17 15:17:19.321
...
ALTER TABLE Bike ADD CONSTRAINT Bike_Owner FOREIGN KEY Bike_Owner (Owner_o_NIK)
    REFERENCES Owner (o_NIK);

-- Reference: Ticket_Bike (table: Bike)
ALTER TABLE Bike ADD CONSTRAINT Ticket_Bike FOREIGN KEY Ticket_Bike (Ticket_t_ID)
    REFERENCES Ticket (t_ID);

-- Reference: Ticket_Customer (table: Ticket)
ALTER TABLE Ticket ADD CONSTRAINT Ticket_Customer FOREIGN KEY Ticket_Customer (Customer_c_IdentityNumber)
    REFERENCES Customer (c_IdentityNumber);

-- Reference: Ticket_Employee (table: Ticket)
ALTER TABLE Ticket ADD CONSTRAINT Ticket_Employee FOREIGN KEY Ticket_Employee (Employee_e_NIK)
    REFERENCES Employee (e_NIK);

-- Reference: Ticket_Returning (table: Returning)
ALTER TABLE Returning ADD CONSTRAINT Ticket_Returning FOREIGN KEY Ticket_Returning (Ticket_t_ID)
    REFERENCES Ticket (t_ID);
